...
async function doAddMessage(is_topic, req, res) {
    var messages = null;
    try {
        messages = toMessages(req.body, req.auth);
    } catch (e) {
...
async function cleanupRetryHashmaps() {
    logger.debug("clean up retry hashmaps ...");
    try {
        await message_broker.cleanup_retries(conf.redis.keyPrefix);
    } catch (e) {
        logger.error("error while cleanup retry hasmaps:", e.message);
    }
}
cleanupRetryHashmaps();
setInterval(cleanupRetryHashmaps, 1000 * 60 * 10);